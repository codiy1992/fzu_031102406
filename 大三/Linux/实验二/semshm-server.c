...
int main(int argc, char** argv)
{
    ...
    pthread_t tid;
    if((argRead.shmid = creatshm(".",58,SHM_SIZE)) == -1)
	return -1;
    if((argWrite.shmid = creatshm(".", 57, SHM_SIZE)) == -1) //创建或者获取共享内存
        return -1;
/*建立进程和共享内存连接*/
    ...
    if((argWrite.semid = creatsem("./", 39, 1, 1)) == -1)//创建信号量以当前目录作为文件名
        return -1;
    if((argRead.semid = opensem("./",40))==-1)
	return -1;
    if(pthread_create(&tid,NULL,WriteThreadFunc,&argWrite)!=0)//创建新线程
	{
    ...
    sem_delete(argWrite.semid); //把semid指定的信号集从系统中删?
    sem_delete(argRead.semid);
